...
func CallRefresh() {
	...
	if err != nil {
		LogMessage(ERRO, "Failed to log message: %v\n")
	...
	if _, err := os.Stat(configPath); err == nil {
		LogMessage(INFO, "Config file exists. Clearing contents.\n")
		// Configファイルが存在する場合、内容をクリア

		// ステップ2: ファイルの内容を空にする
		err = ioutil.WriteFile(configPath, []byte{}, 0644)
		if err != nil {
			LogMessage(ERRO, "Failed to clear config file: %v\n")
			return
		}
		...
		if err != nil {
			LogMessage(ERRO, "Failed to stop or disable the tunnel:  %v\n")
			return
		}
